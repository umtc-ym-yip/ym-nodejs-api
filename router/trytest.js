...
router.get('/oayyieldt', (req, res) => {
    ...
    SELECT Rtrim(lt.lotnum)lotnum,PARTNO,c.Count,ProdClass,LotType lot_type,In_Qnty,Out_Qnty,
                ...
                convert(varchar, e.ChangeTime, 120)ChangeTime,convert(varchar, lt.IOTime, 120) IOTime FROM lt
                INNER JOIN (SELECT lotnum,Count(*)Count FROM PDL_CKHistory(nolock) WHERE lotnum IN (SELECT DISTINCT lotnum FROM lt) group by lotnum)c ON lt.lotnum=c.lotnum
                INNER JOIN (SELECT p.lotnum,ProcName,BefDegree,BefTimes,ChangeTime FROM PDL_CKHistory(nolock)p INNER JOIN ProcBasic b on p.ProcCode=b.ProcCode INNER JOIN minmaxTimes t ON p.lotnum=p.lotnum AND p.ChangeTime=t.MaxTime WHERE p.lotnum IN (SELECT DISTINCT lotnum FROM lt)) b ON lt.lotnum=b.lotnum 
                 INNER JOIN (SELECT p.lotnum,ProcName,BefDegree,BefTimes,ChangeTime FROM PDL_CKHistory(nolock)p INNER JOIN ProcBasic b on p.ProcCode=b.ProcCode INNER JOIN minmaxTimes t ON p.lotnum=p.lotnum AND p.ChangeTime=t.MinTime WHERE p.lotnum IN (SELECT DISTINCT lotnum FROM lt)) e ON lt.lotnum=e.lotnum 
